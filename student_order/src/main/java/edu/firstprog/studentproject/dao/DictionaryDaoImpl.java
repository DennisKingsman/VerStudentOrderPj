...
public class DictionaryDaoImpl implements DictionaryDao
{
    ...
    private static final String GET_STREET = "select street_code, street_name from jc_street where UPPER (street_name) like UPPER(?);"; // ? is parameter
    ...
    public List<Street> findStreets(String pattern) throws DaoException
    {
        ...
        try (Connection connection = getConnection();
             PreparedStatement stmt = connection.prepareStatement(GET_STREET)) // can set parameters
        {
            stmt.setString(1, "%" + pattern + "%"); //setting parameter

            ResultSet res = stmt.executeQuery();
            while (res.next()) {
                Street str = new Street(res.getLong("street_code"), " : " + res.getString("street_name"));
                result.add(str);
            }
    ...
    private String buildParam(String areaId) throws SQLException {
        if (areaId == null || areaId.trim().isEmpty()) //method trim deletes all spaces
        {
            return "__0000000000";
